    ...
    const displayPlaylists = (object) => {
        return(
            <>  
                ...
                </form> : <p style={{display: "inline-block"}}>{object.name}</p>}
                
                {!object.edit && <button style={{display: "inline-block", marginLeft: 10}} type="button" onClick={() => renameClickHandler(object)}>rename</button>}
                <button style={{display: "inline-block", marginLeft: 10}} type="button" onClick={() => moreInfoHandler(object)}>{object.moreInfo ? "^" : "v"}</button>
